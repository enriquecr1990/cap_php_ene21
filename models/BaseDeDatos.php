...
/**
 * Class DataBase que nos ayudara en todas las operaciones de la BD
 * como son los selects, insert, update y deletes
 * el diseño de esta clase la usaremos con la teoria de POO
 */

class BaseDeDatos
{

    //variables privadas para almacenar los datos necesarios de la conexion a la BD
    private $dbConfig;
    private $mysqli;
    private $sentencia;

    /**
     * inicializamos el constructor con la config de la BD
     */
    function __construct(){
        try{
            ...
            if($this->mysqli->connect_errno){
                echo "Error al conectar a MySQL: (" . $this->mysqli->connect_errno . ") " . $this->mysqli->connect_error;die;
            }
        }catch (Exception $ex){
            //mostramos el mensaje del error y nos salimos
            echo 'Error en el constructor del DataBase';die;
        }
    }

    //desctructor de la BD
    function __destruct()
    {
        $this->mysqli->close();
        $this->dbConfig = null;
